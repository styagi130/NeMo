...
def test_codec_cohort_target_honors_bounded_runtime_override(monkeypatch):
    monkeypatch.setenv("EASYMAGPIE_CODEC_COHORT_MAX_BATCH_SIZE", "32")
    assert _codec_cohort_target({"codec_microbatch_max_batch_size": 64}, 64) == 32

    monkeypatch.setenv("EASYMAGPIE_CODEC_COHORT_MAX_BATCH_SIZE", "256")
    assert _codec_cohort_target({"codec_microbatch_max_batch_size": 64}, 64) == 64
