...
@pytest.mark.parametrize("fused_conv_transpose", [False, True])
def test_cuda_packed_kernels_preserve_state_across_chunks(
    monkeypatch: pytest.MonkeyPatch,
    fused_conv_transpose: bool,
) -> None:
    torch.manual_seed(29)
    if fused_conv_transpose:
        monkeypatch.setenv("EASYMAGPIE_CODEC_FUSED_CONV_TRANSPOSE", "1")
    else:
        monkeypatch.delenv("EASYMAGPIE_CODEC_FUSED_CONV_TRANSPOSE", raising=False)
