...
def test_packed_causal_conv1d_h100_tf32(monkeypatch: pytest.MonkeyPatch) -> None:
    """The opt-in H100 path stays within TF32's expected numerical error."""
    monkeypatch.setenv("EASYMAGPIE_CODEC_PACKED_CONV_TF32", "1")
    torch.manual_seed(29)
    device = torch.device("cuda")
    factor = 6
    lengths = [6, 6, 4]
    sequences = [torch.randn(length * factor, 32, device=device) for length in lengths]
    packed = torch.cat(sequences)
    weight = torch.randn(64, 32, 3, device=device)
    bias = torch.randn(64, device=device)
    state = torch.zeros(len(lengths), CODEC_STATE_ELEMENTS, device=device)
    query_start_loc = torch.tensor([0, 6, 12, 16], dtype=torch.int32, device=device)
    cache_indices = torch.arange(len(lengths), dtype=torch.int32, device=device)
    has_initial = torch.zeros(len(lengths), dtype=torch.bool, device=device)
