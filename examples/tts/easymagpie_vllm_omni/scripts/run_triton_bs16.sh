#!/bin/bash
# Launch the EasyMagpie Triton + vLLM-Omni BS16 deployment.
set -euo pipefail
...
MODEL_REPOSITORY="${1:-/home/siddhartht/.cache/easymagpie_triton_bs16}"
HTTP_PORT="${2:-8000}"
GRPC_PORT="$((HTTP_PORT + 1))"
METRICS_PORT="$((HTTP_PORT + 2))"
CONTAINER_NAME="${EASYMAGPIE_TRITON_CONTAINER:-easymagpie-triton-bs16}"
IMAGE="${EASYMAGPIE_TRITON_IMAGE:-easymp-vllm-omni:latest}"

if [[ ! -f "${MODEL_REPOSITORY}/codec/1/model.plan" ]]; then
    echo "Missing BS16 TensorRT codec plan: ${MODEL_REPOSITORY}/codec/1/model.plan" >&2
    exit 2
fi
if [[ ! -f "${MODEL_REPOSITORY}/easymp/1/model.py" ]]; then
    echo "Missing BS16 Triton Python backend: ${MODEL_REPOSITORY}/easymp/1/model.py" >&2
    exit 2
fi
...
echo "Starting EasyMagpie Triton BS16: repo=${MODEL_REPOSITORY}, http=${HTTP_PORT}, grpc=${GRPC_PORT}"
...
exec docker run --rm \
    ...
    -e "EASYMAGPIE_CODEC_MICROBATCH_MAX_BATCH_SIZE=${EASYMAGPIE_CODEC_MICROBATCH_MAX_BATCH_SIZE:-16}" \
    "${IMAGE}" \
    tritonserver \
