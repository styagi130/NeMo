#!/bin/bash
# Launch the EasyMagpie Triton + vLLM-Omni BS32 deployment.
set -euo pipefail
...
MODEL_REPOSITORY="${1:-/home/siddhartht/.cache/easymagpie_triton_bs32}"
HTTP_PORT="${2:-8000}"
GRPC_PORT="$((HTTP_PORT + 1))"
METRICS_PORT="$((HTTP_PORT + 2))"
CONTAINER_NAME="${EASYMAGPIE_TRITON_CONTAINER:-easymagpie-triton-bs32}"
IMAGE="${EASYMAGPIE_TRITON_IMAGE:-easymp-vllm-omni:latest}"

if [[ ! -f "${MODEL_REPOSITORY}/codec/1/model.plan" ]]; then
    echo "Missing BS32 TensorRT codec plan: ${MODEL_REPOSITORY}/codec/1/model.plan" >&2
    exit 2
fi
if [[ ! -f "${MODEL_REPOSITORY}/easymp/1/model.py" ]]; then
    echo "Missing BS32 Triton Python backend: ${MODEL_REPOSITORY}/easymp/1/model.py" >&2
    exit 2
fi
...
echo "Starting EasyMagpie Triton BS32: repo=${MODEL_REPOSITORY}, http=${HTTP_PORT}, grpc=${GRPC_PORT}"
