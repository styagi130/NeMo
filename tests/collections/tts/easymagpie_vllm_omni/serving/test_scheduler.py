...
def test_late_streaming_sentinel_flushes_codec_tail_without_resuming(monkeypatch, tail_frames):
    ...
    saved = []
    scheduler.chunk_transfer_adapter = SimpleNamespace(save_async=lambda **task: saved.append(task))
...
def test_late_close_override_leaves_other_admissions_unchanged(monkeypatch, kind):
    ...
    scheduler.requests = {} if kind == "new" else {request.request_id: SimpleNamespace(status=status)}
    forwarded = []
